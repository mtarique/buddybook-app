...
export default class Search {
    ...
    renderResultsHTML(posts) {
      if(posts.length) {
          ...
          <div class="list-group shadow-sm">
            <div class="list-group-item active"><strong>Search Results</strong> (${posts.length > 1 ? `${posts.length} items found` : '1 item found'})</div> 
              ${posts.map(post => {
                ...
                let formattedPostDate = parseInt(postDate.getMonth()+1)+"/"+postDate.getDate()+"/"+postDate.getFullYear(); 
                return `
                  <a href="/post/${post._id}" class="list-group-item list-group-item-action">
                    <img class="avatar-tiny" src="${post.author.avatar}"> <strong>${post.title}</strong>
                    <span class="text-muted small">${post.author.username} on ${formattedPostDate}</span>
                  </a>
                `; 
              }).join('')}
          </div>
        ...
        this.resultsArea.innerHTML = `<p class="alert alert-danger text-cende shadow-sm">Sorry, we could not find any result for that search.</p>`; 
      }
